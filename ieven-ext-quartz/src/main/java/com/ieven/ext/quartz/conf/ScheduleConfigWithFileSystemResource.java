 ...
 *
 */
public class ScheduleConfigWithFileSystemResource extends AbstractRegisterConfigWithFileSystemListener {
	...
	public ScheduleConfigWithFileSystemResource() {
		// 用来设定需要进行的注册类型
		this(RegistType.SPECIFIC_XPATH, "content/leaf");
	}

	private ScheduleConfigWithFileSystemResource(RegistType registType, String XPath) {
	...
	public void beforeRegister(List<Element> list) {
		...
		XMLUtilWithFileSystemResource xmlUtil = null;
		try {
			xmlUtil = new XMLUtilWithFileSystemResource(resource.getInputStream());
